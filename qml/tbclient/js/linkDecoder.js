...
function loadImage(url){
    if (tbsettings.openWithSystem){
        manager.abortDownload()
        var f = tbsettings.imagePath + "/" + Qt.md5(url) + "." + url.split("\.").pop()
        if (manager.existsFile(f))
            utility.openFileDefault("file:///"+f)
        else {
            Qt.createComponent("Component/ImageShower.qml").createObject(app)
            manager.appendDownload(url, f)
        }
    } else
        pageStack.push(Qt.resolvedUrl("ImagePage.qml"), { imgUrl: url })
}
