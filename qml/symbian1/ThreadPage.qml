...
MyPage {
    ...
    tools: ToolBarLayout {
        ...
        ToolButtonWithTip {
            ...
            visible: tabGroup.currentTab != null;
            onClicked: {
                if (!threadMenu){ threadMenu = menuComp.createObject(page); }
                threadMenu.open();
            }
        }
    }
    ...
    QtObject {
        ...
        function addThreadPage(option){
            ...
            view.threadId = option.threadId;
            view.pageStack = page.pageStack;
            if (option.title) view.title = option.title;
            if (option.isLz) view.isLz = option.isLz;
            if (option.pid) view.getlist(option.pid)
            else view.getlist();

            if (!tabComp) tabComp = Qt.createComponent("Component/ThreadButton.qml");
            var tab = tabComp.createObject(viewHeader.layout);
            tab.tab = view;
        }
